...
For this visualization, we present the sample sizes required for each combination of effect sizes of the main variable of interest (e.g., treatment effect) for each subgroup of the second factor in the design (e.g., sex, age or any systematic heterogenization factor).
...
report3 <- report3%>%
        mutate(ntotal = NA, 
               delete = c(report3$d1 == report3$d2))%>%
        filter(delete == FALSE)
...
Fig1C<- ggplot(data = report3) + 
        ...
        scale_fill_gradient(low="white", high="black", name = "Sample size", trans = "log10")+
