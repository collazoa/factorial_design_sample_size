...
This is the code used to create figure 1 of the manuscript "Mapping strategies towards improved external validity in preclinical translational research" by Carneiro et al. (2023). This version includes some adjustments in response to the anonymous reviewers' comments, which we appreciated and are thankful for.

To select these scenarios to present, we explored multiple options that can be accessed in the GitHub repository: https://github.com/collazoa/factorial_design_sample_size
...
Fig1A<-ggplot(report)+ 
  ...
  labs( x = "factorial design",
        y = "total sample size",
        color = "effect size f",
        title = "Including additional factors") +
  scale_colour_viridis_d() + 
  theme_classic() + 
  scale_y_continuous(limits = c(0,70), n.breaks = 7) + 
  theme(text = element_text(size = 20), legend.text = element_text(size = 16), legend.title = element_text(size = 16))
...
Fig1B<-ggplot(report2)+ 
  ...
  labs( x = "factorial design",
        y = "total sample size",
        color = "effect size f",
        title = "Including additional levels") +
  scale_colour_viridis_d() + 
  theme_classic() + 
...
```{r}
Fig1 = cowplot::plot_grid(Fig1A, Fig1B, labels = LETTERS)
Fig1

ggsave("Figure-1-revised-2nd-revision.png", width = 40, height = 10, units = "cm")
```
...
fig_partial <- design_result_partial_interaction$meansplot + 
  labs(title = "Partially attenuated", x = element_blank(), color = element_blank(), y = "outcome value") +
  theme_classic()  + 
  theme(text=element_text(size=20), legend.position = "none") + 
  scale_y_continuous(limits = c(0,1.2), n.breaks = 3)
...
fig_full <- design_result_fully_interaction$meansplot + 
  labs(title = "Fully attenuated", x = element_blank(), color = element_blank(), y = "outcome value") +
  theme_classic() + 
  theme(text=element_text(size=20), legend.position = "none") + 
  scale_y_continuous(limits = c(0,1.2), n.breaks = 3)
fig_full
...
fig_cross <- design_result_crossover_interaction$meansplot+ 
  labs(title = "Cross-over", x = element_blank(), color = element_blank(), y = "outcome value") +
  theme_classic() + 
  theme(text=element_text(size=20), legend.position = "none") + 
  scale_y_continuous(limits = c(0,1.2), n.breaks = 3)
fig_cross
...
fig_no <- design_result_no_interaction$meansplot+ 
  labs(title = "No interaction", x = element_blank(), color = element_blank(), y = "outcome value") +
  theme_classic() + 
  theme(text=element_text(size=20), legend.text = element_text(size = 16), legend.position = c(0.2, 0.93)) + 
  scale_y_continuous(limits = c(0,1.2), n.breaks = 3)
fig_no

#complete figure
Fig2A = cowplot::plot_grid(fig_no, fig_partial, fig_full, fig_cross, nrow = 1)
Fig2A
```
...
Fig2B<- ggplot(data = report3) + 
        ...
        geom_rect(aes(
            ...
            ymin = -1.05,
            ymax = -0.05),
          fill = NA, color = "#21908c", linewidth = 0.5, linetype = "dashed") + 
        geom_rect(aes(
            xmin = -0.05,
            xmax = -1.05,
            ymin = 1.05,
        ...
        geom_rect(aes(
            ...
            ymin = -1.05,
            ymax = -0.05),
          fill = NA, color = "yellow", linewidth = 0.5, linetype = "dashed") +
        geom_rect(aes(
            xmin = 1.05,
            xmax = 0.05,
            ymin = 1.05,
            ymax = 0.05),
          fill = NA, color = "yellow", linewidth = 0.5, linetype = "dashed") + 
          geom_richtext(aes(x = -0.55, y=1, label= "**cross-over**"), col="#21908c") +
          geom_richtext(aes(x=0.55, y=1, label= "**partially attenuated**"), col="#d4b739") +
          geom_richtext(aes(x = 0.55, y=-1, label= "**cross-over**"), col="#21908c") +
          geom_richtext(aes(x=-0.55, y=-1, label= "**partially attenuated**"), col="#d4b739") +
    theme(text = element_text(size = 20), legend.text = element_text(size = 16), legend.title = element_text(size = 16))
...
```
```{r}
Fig2C = ggplot(report4) +
  geom_point(aes(x = d_a, y = n, color = size_i, shape = effect), size = 3)+ 
  scale_colour_viridis_d(name = "relative interaction ES",
                         labels = c("1/2 of main ES",
                                    "1/3 of main ES",
                                    "1/4 of main ES")) +  
  geom_point(aes(x = d_a, y = n, shape = effect), size = 3, data = report4 %>% filter(effect=="n_main")) + 
  scale_shape_discrete(name = "effect size (ES) of interest",
                       labels = c("interaction ES",
                                  "main ES")) + 
...
```{r}
Fig2_bottom = cowplot::plot_grid(Fig2B, Fig2C, nrow = 1, labels = c("B", "C"))

Fig2_complete = cowplot::plot_grid(Fig2A, Fig2_bottom, nrow = 2, labels = c("A", ""))
Fig2_complete

ggsave("Figure-2-revised-2nd-revision.png", width = 40, height = 20, units = "cm")
```